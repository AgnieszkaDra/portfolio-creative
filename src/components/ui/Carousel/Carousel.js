...
export const Carousel = (props) => {
  ...
  const handleSpanIncrease = () => {
      setCurrentSlide((prevSlide) => (prevSlide > 0 ? prevSlide - 1 : projects.categories.length - 1));
  };

  const handleSpanDecrease = () => {
      setCurrentSlide((prevSlide) => (prevSlide < projects.categories.length - 1 ? prevSlide + 1 : 0));
  };
